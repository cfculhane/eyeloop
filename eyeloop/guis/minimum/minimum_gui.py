...
class GUI:
    def __init__(self, engine: Engine, importer: Importer, markers: bool = False) -> None:
        self.engine = engine
            ...
            if key == "p":
                ...
                try:
                    ...
                    self.pupil_processor.reset(self.cursor, np.mean(
                        self.engine.source[self.cursor[1] - 1:self.cursor[1] + 1,
    ...
    def arm(self, width: int, height: int) -> None:
        ...
        self.pupil_processor = self.engine.pupil_processor
    ...
    def update_track(self, blink: int) -> None:
        ...
        if self._state == "adjustment":
            ...
            cv2.putText(frame_source, "#" + str(self.importer.frame), (to_int(self.width / 2), 12), font, .7,
